...
TritonGPULLVMToX86TypeConverter::TritonGPULLVMToX86TypeConverter(
    MLIRContext *ctx, LowerToLLVMOptions &option,
    const DataLayoutAnalysis *analysis)
    : LLVMTypeConverter(ctx, option, analysis) {

  addConversion([&](LLVM::LLVMPointerType type)
                    -> std::optional<Type> {
  ...
  addConversion([&](LLVM::LLVMStructType structTy)
                  -> std::optional<Type> {
    ...
    for (Type t : structTy.getBody()) {
      auto conv = convertType(t);
      // llvm::errs() << "Converting struct element type: " << t  << " to "<< conv << "\n";
      if (conv) {
        elems.push_back(conv);
        changed |= (conv != t);
      } else {
        elems.push_back(t);
      }
    ...
    if (structTy.isIdentified()) {
      // llvm::errs() << "Rebuilding identified struct: " << structTy.getName() << "\n";
      auto newStruct = LLVM::LLVMStructType::getIdentified(ctx,
                                                       structTy.getName());
      auto ret = newStruct.setBody(elems, structTy.isPacked());
      return newStruct;
    }else{
      // llvm::errs() << "Rebuilding anonymous struct\n";
      return LLVM::LLVMStructType::getLiteral(ctx, elems, 
                                              structTy.isPacked());
    }
