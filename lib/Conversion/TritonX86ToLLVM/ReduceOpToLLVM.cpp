...
//===----------------------------------------------------------------------===//
//  Helpers for “metrics” (alloc + counters + dummy‐replace).
//  We assume “N = 2”:  slot 0 will be “shuffleSteps” and slot 1 will be “nativeSteps”.
//  At runtime each thread will bump these two counters as needed.
//
//  Usage inside your conversion (matchAndRewrite):
//   1) Call `Value metricsAlloca = ensureMetricsAlloc(rewriter, llvmFunc, loc);` once.
//   2) Call `incrementMetric(rewriter, loc, metricsAlloca, kShuffleSlot, 1);` 
//      or `incrementMetric(..., kNativeSlot, 1);` wherever you need.
//   3) At the very end, do `replaceReduceWithDummy(rewriter, op, metricsAlloca);`
//      instead of erasing the ReduceOp.
//
//  (You can expand N or add more slots exactly the same way.)
//===----------------------------------------------------------------------===//

static constexpr unsigned kNumMetricsSlots = 4;
static constexpr unsigned kShuffleSlot     = 0;
static constexpr unsigned kNativeSlot      = 1;
