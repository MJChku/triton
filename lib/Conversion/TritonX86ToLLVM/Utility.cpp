...
namespace triton::gpu {
  ...
  /// replaceOpWithDummy
  ///
  ///   Replaces *any* Operation’s results with “dummy” values (undef or zero).
  ///
  ///   For each result type:
  ///     • If it’s a RankedTensorType (static shape), flatten it to a 1D vector
  ///       of length = tensorTy.getNumElements(), element type = tensorTy.getElementType().
  ///     • If it’s a float/int scalar, emit a single zero‐constant.
  ///     • Otherwise, emit an error at op’s location and stop.
  ///
  ///   After forming one dummy per result, calls `rewriter.replaceOp(op, dummies)`.
  ///===----------------------------------------------------------------------===//
  void replaceOpWithDummy(RewriterBase &rewriter,
                                const LLVMTypeConverter &typeConverter,
                                Operation *op) {
    Location loc = op->getLoc();
    SmallVector<Value, 4> dummies;
    dummies.reserve(op->getNumResults());

    for (Type resultTy : op->getResultTypes()) {
      Value dummyForThisResult = nullptr;

      if (auto tensorTy = mlir::dyn_cast<RankedTensorType>(resultTy)) {
        // Statically‐shaped tensor → flatten into a 1D vector of its element type
        ...
        // If you need to repack that flat vector into a memref‐style descriptor,
        // call your packLLElementsIntoMemRef(...) here before pushing into dummies.
        ...
        // Unsupported type → emit an MLIR error
        op->emitError("replaceOpWithDummy: unsupported result type '")
            << resultTy << "'";
        return;
      }

      dummies.push_back(dummyForThisResult);
    }
  ...
  void replaceOpWithDummyPacked(ConversionPatternRewriter &rewriter,
                                      ...
                                      Type elemType,
                                      Type packedType) {
    llvm::errs() << "[replaceOpWithDummyPacked] op: " << *op << "\n";
    
    Location loc = op->getLoc();

    SmallVector<Value> flatDummies;
    flatDummies.reserve(totalElems);
    ...
    for (unsigned i = 0; i < totalElems; ++i) {
      ...
      flatDummies.push_back(oneDummy);
    }
    
    Value packed =
        mlir::packLLElements(loc, &typeConverter, flatDummies, rewriter, packedType);

    rewriter.replaceOp(op, packed);
  }

  Value ensureMetricsAlloc(
  ...
  // ─────── If the function doesn't have our marker, insert a new alloca ───────
  if (!llvmFunc->hasAttr(kFuncMarkerName)) {
    Block &entryBlock = llvmFunc.front();
    ...
    Value cLong = rewriter.create<LLVM::ConstantOp>(
        loc, i64Ty, rewriter.getI64IntegerAttr(kNumMetricsSlots));
    ...
    allocaOp->setAttr(kFuncMarkerName, UnitAttr::get(ctx));

    // Mark the function so we don’t re-insert another alloca here
    llvmFunc->setAttr(kFuncMarkerName, UnitAttr::get(ctx));

  } else {
    // ────── If the function is already marked, find the alloca by that attr ──────
    Block &entryBlock = llvmFunc.front();
    for (Operation &inner : entryBlock) {
      if (auto allocaOp = dyn_cast<LLVM::AllocaOp>(inner)) {
        if (allocaOp->hasAttr(kFuncMarkerName)) {
          metricsAlloca = allocaOp.getResult();
          break;
        }
      }
    }
    assert(metricsAlloca && "Expected existing metrics alloca in entry block");
  }

  return metricsAlloca;
}

  /// Emit (at *current* insertion point) a read‐modify‐write that increments
  /// `metricsAlloca[idx] += amount`.  Does:
  ///   %old = llvm.load i32, i32* getelementptr(metricsAlloca, idx)
  ///   %new = llvm.add %old, constant(amount)
  ///   llvm.store %new, <that GEP>
  ///
  void incrementMetric(RewriterBase &rewriter,
                      Value metricsAlloca, // i32*
                      Location loc,
                      unsigned slotIdx,    // which slot to bump
                      uint32_t amount)  {   // how much to add
   
    // Build “idx : i64” constant
    Value cIndex = rewriter.create<LLVM::ConstantOp>(
        loc, rewriter.getI64Type(),
        rewriter.getI64IntegerAttr((int64_t)slotIdx));
    // GEP: i32* = getelementptr(metricsAlloca, idx)
    auto gep = rewriter.create<LLVM::GEPOp>(
        loc,
        /*resultType=*/metricsAlloca.getType(),   // i32*
        /*elementType=*/rewriter.getI32Type(), // i32
        /*basePtr=*/metricsAlloca,
        /*indices=*/ArrayRef<Value>{cIndex},
        /*inbounds=*/false);
    Value old = rewriter.create<LLVM::LoadOp>(loc, rewriter.getI32Type(), gep);
    Value cAmt = rewriter.create<LLVM::ConstantOp>(
        loc, rewriter.getI32Type(),
        rewriter.getI32IntegerAttr((int32_t)amount));
    Value sum = rewriter.create<LLVM::AddOp>(
        loc, rewriter.getI32Type(), old, cAmt);
    rewriter.create<LLVM::StoreOp>(loc, sum, gep);
  }
